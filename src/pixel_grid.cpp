...
//********************************************
//Run through the lighting sequence for squared
void squared_pixels(uint32_t c)
{
  //**********************************************
  //Check if enough time has passed between frames
  if(time_since(begin) < SQUARED_FRAMERATE)
  {
    return;
  }
  ...
  for(uint8_t i=0; i<grid.numPixels(); i++)
  {
    if(color%2)
    {
      if(squared_lut[1][i] >= 1)   //Outside square
      {
        grid.setPixelColor(pixel_map[i],c);
      }
      else
      {
    ...
    {
      if(squared_lut[0][i] >= 1)   //Inside square
      {
        grid.setPixelColor(pixel_map[i],c);
      }
      else
